...
export async function createDevServer(options: DevServerOptions): Promise<DevServer> {
  ...
  if (vibeDebug) {
    ...
    const corePath = tryResolve('@float-v/core', rootDir);
...
`;
  ...
  async function handleRequest(
    ...
    try {
      ...
      // Handle local workspace packages
      if (pathname.startsWith('/__workspace/')) {
        const parts = pathname.split('/');
        let workspacePkg = '';
        let subPath = '';

        if (parts[2].startsWith('@')) {
          workspacePkg = parts[2] + '/' + parts[3];
          subPath = parts.slice(4).join('/');
        } else {
          workspacePkg = parts[2];
          subPath = parts.slice(3).join('/');
        }

        // Find the physical path of the workspace package
        // First try node_modules (where pnpm links them)
        const pkgPath = path.join(process.cwd(), 'node_modules', workspacePkg);
        if (fs.existsSync(pkgPath)) {
          const realPkgPath = fs.realpathSync(pkgPath);
          // Resolve the actual file (might be .tsx instead of .js)
          const filePath = resolveFilePath(realPkgPath, subPath);

          if (filePath && fs.existsSync(filePath)) {
            console.log(pc.cyan(`  🏠 Workspace: ${workspacePkg}/${subPath}`));
            try {
              const fileContent = fs.readFileSync(filePath, 'utf-8');
              const esbuild = await import('esbuild');
              const result = await esbuild.transform(fileContent, {
              ...
              console.error(pc.red('Workspace transform error:'), error);
              res.writeHead(500);
              res.end('Transform error');
              return;
            }
      ...
      // Handle /__float/ internal assets
      if (pathname.startsWith('/__float/')) {
        ...
        if (pathname === '/__float/hydrate.js') {
          try {
            ...
            res.end(`
// Float-V Client-Side Hydration Runtime
import React from 'react';
import { createRoot } from 'react-dom/client';

console.log('[Float-V] Client hydration starting...');

const data = window.__FLOAT_HYDRATION_DATA;
if (!data) {
  console.log('[Float-V] No hydration data found');
} else {
  console.log('[Float-V] Hydration data found:', data);
  
  // Find the client root element
  const clientRoot = document.getElementById('__float_client_root');
  
  if (clientRoot) {
    console.log('[Float-V] Client root found, loading component...');
    ...
    if (appIndex === -1) {
      console.error('[Float-V] Could not find /app/ in path:', componentPath);
    } else {
      ...
      console.log('[Float-V] Component path: ' + componentPath);
      console.log('[Float-V] Importing from: /' + modulePath);
      
      // Dynamically import the component (needs leading slash)
      import('/' + modulePath).then((module) => {
        console.log('[Float-V] Component loaded successfully');
        const Component = module.default;
        
        if (!Component) {
          console.error('[Float-V] No default export found in component');
          return;
        }
        ...
        console.log('[Float-V] Component rendered successfully!');
      }).catch((error) => {
        console.error('[Float-V] Failed to load component: ' + String(error));
      });
    }
  } else {
    console.log('[Float-V] No client root element found, full hydration...');
    // Fallback to full page hydration
    const container = document.getElementById('__float');
    if (container) {
      console.log('[Float-V] Hydrating full page');
    }
  }
}
            `);
        ...
        res.end('// Float-V internal asset');
        return;
      }
      ...
      // Internal Float-V pages (docs, learn, examples)
      if (pathname === '/__docs') {
...
function resolveFilePath(rootDir: string, pathname: string): string {
  ...
  const strippedPath = cleanPath.replace(/\.(js|mjs)$/, '');

  const extensions = ['', '.tsx', '.ts', '.jsx', '.js'];
  const subDirs = ['', 'src', 'dist'];

  for (const subDir of subDirs) {
    // Try extensions (including empty string for direct match)
    for (const ext of extensions) {
      const tryPath = path.join(rootDir, subDir, (ext === '' ? cleanPath : strippedPath) + ext);
    ...
    for (const ext of extensions.slice(1)) {
      const tryIndexPath = path.join(rootDir, subDir, strippedPath, 'index' + ext);
...
<head>
  ...
  <title>404 - Not Found | Float-V</title>
  <style>
